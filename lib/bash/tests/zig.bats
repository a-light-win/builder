...
@test "zig-pre-build-image should download the zig binary" {
  ...
  [ -e zig-x86_64-linux-${ZIG_VERSION} ] && rmdir zig-x86_64-linux-${ZIG_VERSION}
  ...
  assert_line --index 3 "zig-x86_64-linux-${ZIG_VERSION}.tar.xz"
  assert_line --index 4 "https://ziglang.org/download/${ZIG_VERSION}/zig-x86_64-linux-${ZIG_VERSION}.tar.xz"

  assert_line --index 5 "tar"
  assert_line --index 6 "xf"
  assert_line --index 7 "zig-x86_64-linux-${ZIG_VERSION}.tar.xz"
}
...
@test "zig-pre-build-image should not download the zig binary if it exists" {
  ...
  export ZIG_VERSION=0.13.0
  mkdir -p zig-x86_64-linux-${ZIG_VERSION}

  run zig-pre-build-image x86_64
  assert_success
  assert_line --index 0 "The zig-x86_64-linux-${ZIG_VERSION} already downloaded, skipping"
  refute_line "curl"
  refute_line "tar"

  rmdir zig-x86_64-linux-${ZIG_VERSION}
}
